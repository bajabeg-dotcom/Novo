"""Loader for REAL KORG PA800 factory .mid material under
data/factory/real/.

Hardcodes source_kind=REAL at the call site -- never accepts
...
def import_real_dataset(
    root: str | Path = config.FACTORY_REAL_ROOT,
    conn: sqlite3.Connection | None = None,
) -> ImportSummary:
    """Ingest real Factory Style .mid files under ``root`` into
    evidence.db, hardcoding source_kind='REAL'.
    """
    ...
    try:
        return _ingest_common.ingest(root, conn, source_kind="REAL")
    finally:
        if owns_conn:
            conn.close()
